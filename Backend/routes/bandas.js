...
router.post(
  "/",
  [
    check("nombre", "El nombre es inválido")
      .not()
      .isEmpty()
      .isLength({ min: 1, max: 50 }),
    check("tipo", "El tipo de banda es obligatorio")
      .not()
      .isEmpty()
      .custom((value) => {
    ...
    check("localidad", "La localidad es inválida")
      .not()
      .isEmpty()
      .isLength({ min: 1, max: 50 }),
    check("provincia", "La provincia es inválida")
      .not()
      .isEmpty()
      .isLength({ min: 1, max: 50 }),
    check("codigo_postal", "El código postal es inválido").isPostalCode("ES"),
    check("direccion", "La dirección es inválida")
      .not()
      .isEmpty()
      .isLength({ min: 1, max: 150 }),
    check("año_fundacion", "El año de fundación no es válido")
      .not()
      .isEmpty()
      .custom((value) => {
    ...
    check("descripcion", "La descripción es inválida")
      .not()
      .isEmpty()
      .isLength({ min: 1, max: 500 }),
    check("telefono", "El número de teléfono es inválido")
      .isMobilePhone()
      .isLength({ min: 9, max: 9 }),
    check("correo", "El email es inválido")
      .isEmail()
      .isLength({ min: 1, max: 150 }),
    check("cif", "El CIF es inválido")
      .not()
      .isEmpty()
      .custom((value) => {
...
router.get(
  "/pertenece/usuario/:usuarioId/banda/:bandaId",
  getPerteneceUsuarioBanda
);
