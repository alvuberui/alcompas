...
router.post(
  "/",
  [
    check("tipoEstudio", "El tipo de estudio no es válido")
      .notEmpty()
      .custom((value) => {
        const lista = [
          "Grado elemental",
    ...
    check("centroEstudios", "El centro de estudios no es válido").isLength({
      max: 50,
    }),
    check(
      "poblacion",
      "La población es no puede tener más de 50 caracteres"
    ).isLength({ max: 50 }),
    check(
      "provincia",
      "La provincia no puede tener más de 50 caracteres"
    ).isLength({ max: 50 }),
    check("fechaInicio", "La fecha de inicio no es válida")
      .notEmpty()
      .custom((value) => {
