...
router.post(
  "/",
  [
    check("respuesta", "La respuesta no es válida")
      .notEmpty()
      .custom((value) => {
    ...
    check("tipo", "El tipo de evento no es válido")
      .notEmpty()
      .custom((value) => {
    ...
    check(
      "comentario",
      "El comentario no puede contener más de 200 caracteres"
    ).isLength({ max: 200 }),
...
router.put(
  "/:id",
  [
    check("respuesta", "El tipo de estudio no es válido")
      .notEmpty()
      .custom((value) => {
        const lista = ["Asisto", "No asisto"];
        if (!lista.includes(value)) {
...
router.get(
  "/usuario/:usuarioId/evento/:eventoId/tipo/:tipoEvento",
  getAsistenciaByUsuarioEventoAndTipo
);
