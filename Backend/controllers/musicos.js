...
const finalizarMusico = async (req, res = express.response) => {
  try {
    ...
    const musico = await Musico.find({
      usuario: userId,
      banda: bandaId,
  ...
  try {
    ...
    for (i = 0; i < musicos.length; i++) {
      ...
      if (diccionario[musico.instrumento] == undefined) {
        ...
        diccionario[musico.instrumento] = [
          ...diccionario[musico.instrumento],
          musico,
        ];
      }
...
/*
    Obtener músicos por instrumento y localidad
*/
const getMusicosByIntrumentoAndLocalidad = async (
  req,
...
/*
    Comprobar si es músico de una banda
*/
