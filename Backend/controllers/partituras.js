...
const verMisParituras = async(req, res = express.response) => {
    try {
        ...
        // muestran todas o no.
        const archiveros = await Archivero.find({"usuario": payloadId, "banda": repertorio.banda, "fecha_final": undefined});
        const directivos = await Directivo.find({"usuario": payloadId, "banda": repertorio.banda, "fecha_final": undefined});
        let partituras = [];
        ...
        const musicos = await Musico.find({"usuario": payloadId, "banda": repertorio.banda, "fecha_final": undefined});

        if(musicos.length > 0){
            partituras = await Partitura.find({"obra": obraId, "instrumento": musicos[0].instrumento});
            return res.json({
                ok: true,
                partituras
            });
        ...
        if(archiveros.length === 0 && directivos.length === 0 && musicos.length === 0){
            return res.status(401).json({
                ok: false,
...
const getPartituraById = async(req, res = express.response) => {
    try {
        ...
        if(musicos.length === 0 && archiveros.length === 0 && directivos.length === 0){
            return res.status(401).json({
                ok: false,
                msg: 'No tiene privilegios para ver partituras'
            });
