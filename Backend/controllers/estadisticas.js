...
const getBandasConMasContratosSemanaSanta = async(req, res = express.response) => {
    try {
        ...
        for(let i = 0; i < bandas.length; i++) {
            ...
            const procesiones = await Procesion.find({banda: banda._id, tipo: 'Semana Santa', fechaInicio: {$gte: new Date(), $lte: new Date(new Date().setFullYear(new Date().getFullYear() + 1))},
                        fechaFin: {$gte: new Date(), $lte: new Date(new Date().setFullYear(new Date().getFullYear() + 1))}});
...
const getBandasConMasContratos = async(req, res = express.response) => {
    try {
        ...
        for(let i = 0; i < bandas.length; i++) {
            const banda = bandas[i];
            // Obtener numero de procesiones de la banda desde ahora a dentro de un año
            const procesiones = await Procesion.find({banda: banda._id, fechaInicio: {$gte: new Date(), $lte: new Date(new Date().setFullYear(new Date().getFullYear() + 1))},
                        fechaFin: {$gte: new Date(), $lte: new Date(new Date().setFullYear(new Date().getFullYear() + 1))}});
            const actuaciones = await Actuacion.find({banda: banda._id, fechaInicio: {$gte: new Date(), $lte: new Date(new Date().setFullYear(new Date().getFullYear() + 1))},
                        fechaFin: {$gte: new Date(), $lte: new Date(new Date().setFullYear(new Date().getFullYear() + 1))}});
