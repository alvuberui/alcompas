...
const crearPeticion = async (req, res = express.response) => {
  ...
  try {
    ...
    for (const peticiondbl of peticiones) {
      if (
        peticiondbl.estado == "Pendiente" &&
        peticiondbl.rol == peticion.rol
      ) {
        return res.status(400).json({
          ok: false,
    ...
    if (peticion.rol == "Directivo") {
      const roles = await Directivo.find({
        usuario: usuarioId,
        banda: bandaId,
      });
    ...
    if (peticion.rol == "Archivero") {
      const roles = await Archivero.find({
        usuario: usuarioId,
        banda: bandaId,
      });
      for (const rol of roles) {
    ...
    if (
      peticion.rol === "Músico" &&
      (peticion.instrumento === undefined ||
        peticion.instrumento === "" ||
        peticion.voz === "" ||
        peticion.voz === undefined ||
        peticion.cargo !== undefined)
    ) {
      return res.status(400).json({
        ok: false,
...
const aceptarPeticion = async (req, res = express.response) => {
  ...
  try {
    ...
    const peticionAceptada = await Peticion.findByIdAndUpdate(
      peticion._id,
      nuevaPeticion,
      { new: true }
    );
    ...
    if (peticion.rol == "Músico") {
      const musico = new Musico({
        fecha_inicio: new Date(),
        instrumento: peticion.instrumento,
        voz: peticion.voz,
        usuario: peticion.usuario,
        banda: peticion.banda,
      });
...
const rechazarPeticion = async (req, res = express.response) => {
  ...
  try {
    ...
    const peticionRechazada = await Peticion.findByIdAndUpdate(
      peticion._id,
      nuevaPeticion,
      { new: true }
    );
