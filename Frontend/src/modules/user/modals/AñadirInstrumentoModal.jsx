...
export const AñadirInstrumentoModal = ({
  open,
  handleClose,
  setInstrumentos,
  setOpen,
  ...
  const validarInstrumento = () => {
    let error = "";
    if (validarInstrumentos(instrumento.instrumento) === false)
      error = error + " El instrumento no es válido ";
    if (instrumento.modelo.length > 50)
      error =
        error + " <br>  El modelo no puede contener más de 50 caracteres ";
    if (instrumento.marca.length > 50)
      error = error + " <br>  La marca no puede contener más de 50 caracteres ";
    if (instrumento.numeroSerie.length > 50)
      error =
        error +
        " <br>  El número de serie no puede contener más de 50 caracteres ";
  ...
  const handleFormEditar = async (e) => {
    ...
    if (error != "") {
      ...
      if (banda) {
        const c = await editarInstrumentoBanda(
          instrumento,
          banda,
          instrumentoId
        );
        setInstrumentos((co) =>
          [...co, c].filter((i) => i._id !== instrumentoId)
        );
        ...
        const c = await editarInstrumentoUsuario(
          instrumento,
          user.uid,
          instrumentoId
        );
        ...
        <Box sx={style}>
          <form>
            ...
            <Grid container>
              ...
              <Grid item xs={12} sx={{ mt: 2 }}>
                <Typography sx={{ color: "white" }}>Marca</Typography>
                <TextField
                ...
                />
                <Typography sx={{ color: "white", mt: 2 }}>Modelo</Typography>
                <TextField
                  sx={{ input: { color: "white" } }}
                  inputProps={{ style: { color: "white" } }}
                  type="text"
