...
export const AñadirEstudioModal = ({
  open,
  handleClose,
  setEstudios,
  setOpen,
  ...
  // Funciones
  const { crearEstudio, errores, setErrores, getEstudioById, editarEstudio } =
    useEstudiosStore();
  ...
  const validarEstudio = () => {
    ...
    if (!tipos.includes(estudio.tipoEstudio))
      error = "El tipo de estudio no es válido";
    if (estudio.fechaInicio > estudio.fechaFin)
      error =
        error +
        "<br> La fecha de inicio no puede ser mayor que la fecha de fin";
    if (estudio.fechaInicio === "")
      error = error + "<br> La fecha de inicio no puede estar vacía";
    if (estudio.fechaFin === "")
      error = error + "<br> La fecha de fin no puede estar vacía";
    if (estudio.fechaInicio > fechaActual)
      error =
        error +
        "<br> La fecha de inicio no puede ser mayor que la fecha actual";
    if (estudio.fechaFin <= estudio.fechaInicio)
      error =
        error +
        "<br> La fecha final no puede ser igual o anterior a la fecha de inicio";
    return error;
  };
          ...
          <form>
            ...
            <Grid container>
              ...
              <Grid item xs={12}>
                ...
                />
                <Typography sx={{ color: "white" }}>Población</Typography>
                <TextField
                  sx={{ input: { color: "white" } }}
                  inputProps={{ style: { color: "white" } }}
                  type="text"
                ...
                />
                <Typography sx={{ color: "white" }}>Provincia</Typography>
                <TextField
                  sx={{ input: { color: "white" } }}
                  inputProps={{ style: { color: "white" } }}
                  type="text"
                ...
                />
                <Typography sx={{ color: "white" }}>Fecha de inicio</Typography>
                <TextField
                  sx={{ input: { color: "white" } }}
                  inputProps={{ style: { color: "white" } }}
                  type="date"
