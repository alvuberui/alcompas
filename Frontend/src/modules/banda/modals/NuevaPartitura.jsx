...
import { useObrasStore, usePartiturasStore, useRepertoriosStore } from '../../../hooks';
const style = {
    position: 'absolute',
    top: '50%',
    left: '50%',
...
export const NuevaPartitura = ({ open, handleClose, setPartituras, setOpen, obra }) => {
    ...
    const [values, setValues] = useState({instrumento:'Clarinete', voz: 'Primero'});
    ...
    const handleForm = async e => {
        ...
        if( !instrumentos.includes(values.instrumento) ) error = error + " El instrumento no es válido";
        if( !voces.includes(values.voz) ) error = error + " <br>  La voz no es válida";
        if( nombre === '' || partitura === '') error = error + " <br>  Es obligatorio subir un archivo";
