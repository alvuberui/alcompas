...
import { useAnunciosStore, useComentariosStore } from '../../../hooks';
const style = {
    position: 'absolute',
    top: '50%',
    left: '50%',
...
export const NuevoAnuncio  = ( { open, handleClose, setAnuncios, setOpen }) => {
    ...
    const [values, setValues] = useState({titulo:'', descripcion: '', privacidad:'Pública'});
    ...
    const handleForm = async e => {
        ...
        if(values.titulo.length > 75 || values.titulo.length < 1 ) error = error + " El título debe de contener entre 1 y 75 caracteres ";
        if(values.descripcion.length > 2000 || values.descripcion.length < 1 ) error = error + " <br>  El texto debe de contener entre 1 y 2000 caracteres";
        if(!lista.includes(values.privacidad)) error = error + " <br>  La privacidad debe de ser Pública, Privada o Restringida";
        if(error != "") {
