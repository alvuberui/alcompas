...
export const PublicarAsistenciaModal = ({ open, handleClose, evento, setOpen, tipo }) => {
    ...
    const [values, setValues] = useState({respuesta:'Asisto', tipo: evento.tipo, comentario:'', referencia: evento._id, usuario: user.uid });
    ...
    const handleForm = async e => {
        ...
        values.tipo = tipo === 'Procesión' ? 'Procesion' : tipo === 'Actuación' ? 'Actuacion' : 'Ensayo';
        if(values.comentario > 200  ) error = error + " El nombre debe de contener menos de 200 caracteres";
        if( !lista.includes(values.respuesta) ) error = error + " <br>  La respuesta no es válida";
      ...
      useEffect(() => {
        setValues({respuesta:'Asisto', tipo: evento.tipo, comentario:'', referencia: evento._id, usuario: user.uid });
        }, [evento])

        useEffect(() => {
            const getAsistencia = async () => {
                if( user.uid && evento._id && tipo) {
                    const t = tipo === 'Procesión' ? 'Procesion' : tipo === 'Actuación' ? 'Actuacion' : 'Ensayo';
                    const asistencia = await getAsistenciaByUsuarioEventoAndTipo(user.uid, evento._id, t);
          ...
          <Box sx={style} >
            ...
            <form>
                ...
                <Grid item xs={ 12 } sx={{ mt: 2}}>
                    ...
                    >
                      <MenuItem key={'Asisto'} value={'Asisto'}>Asisto</MenuItem>
                      <MenuItem key={'No asisto'} value={'No asisto'}>No asisto</MenuItem>
                    </Select>
                    </Grid>
                    <Grid item xs={ 12 } sx={{ mt: 2}}>
                    <Typography  sx={{ color:'white'}} >
                ...
                <Box  sx={{mt:2, display:'flex', alignContent:'center', justifyContent:'center'}} >
                    { modo === 'crear' ?
                      ...
                      :
                        <Button aria-label='enviar' color='secondary' sx={{ backgroundColor:'white', color:'black'}} variant='contained' onClick={handleUpdate}>
