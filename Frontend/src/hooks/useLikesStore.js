...
export const useLikesStore = () => {
    ...
    const publicarDislike = async(like) => {
        try {
            const { data } = await alcompasAPI.delete('likes/tipo/' + like.tipo + '/referencia/' + like.referencia );
        }
        catch(error) {
    ...
    const getLikeByTipoAndReferencia = async(likes) => {
        try {
            const { data } = await alcompasAPI.get('likes/tipo/' + likes.tipo + '/referencia/' + likes.referencia);
    ...
    const getNumeroLikes = async(likes) => {
        try {
            const { data } = await alcompasAPI.get('likes/numero/tipo/' + likes.tipo + '/referencia/' + likes.referencia);
